...
garbage :: Parsec String () Int
garbage = char '<' *> many (char '!' *> anyChar <|> noneOf "!>") *> char '>' *> return 0

group :: Int -> Parsec String () Int
group n = (+n) <$> (char '{' *> (sum <$> (garbage <|> group (n+1)) `sepBy` char ',') <* char '}')
...
-- >>> part1 "example 4" "{{{},{},{{}}}}"
-- Right 16
-- >>> part1 "example 5" "{<a>,<a>,<a>,<a>}"
-- Right 1
-- >>> part1 "example 6" "{{<ab>},{<ab>},{<ab>},{<ab>}}"
-- Right 9
-- >>> part1 "example 7" "{{<!!>},{<!!>},{<!!>},{<!!>}}"
-- Right 9
-- >>> part1 "example 8" "{{<a!>},{<a!>},{<a!>},{<ab>}}"
-- Right 3
part1 :: SourceName -> String -> Either ParseError Int
part1 = runParser (group 1) ()
...
-- |
-- >>> runParser garbage' () "example" "<>"
-- Right 0                   
-- >>> runParser garbage' () "example" "<random characters>"
-- Right 17                  
-- >>> runParser garbage' () "example" "<<<<>"
-- Right 3                   
-- >>> runParser garbage' () "example" "<{!>}>"
...
-- >>> runParser garbage' () "example" "<!!!>>"
-- Right 0                   
-- >>> runParser garbage' () "example" "<{o\"i!a,<{i<a>"
-- Right 10
garbage' :: Parsec String () Int
garbage' = char '<' *> (sum <$> many (0 <$ (char '!' *> anyChar) <|> 1 <$ noneOf "!>")) <* char '>'

group' :: Parsec String () Int
group' = char '{' *> (sum <$> (garbage' <|> group') `sepBy` char ',') <* char '}'

main :: IO ()
main = print =<< parseFromFile group' "Day9.input"
