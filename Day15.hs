...
-- |
-- >>> take 5 $ values gena 65
-- [1092455,1181022009,245556042,1744312007,1352636452]
-- >>> take 5 $ values genb 8921
-- [430625591,1233683848,1431495498,137874439,285222916]
values :: Int -> Int -> [Int]
values gen = (tail.) . iterate $ \x -> (gen * x) `rem` base

-- X
-- >>> judge 65 8921
-- 588
judge :: Int -> Int -> Int
judge inia inib = length . filter id . take 40000000 $ zipWith 
  (\a b -> mask .&. a == mask .&. b)
  (values gena inia)
  (values genb inib)
...
valuesa :: Int -> [Int]
valuesa = filter (\a -> a `rem` 4 == 0) . values gena

-- |
-- >>> take 5 $ valuesb 8921
-- [1233683848,862516352,1159784568,1616057672,412269392]
valuesb :: Int -> [Int]
valuesb = filter (\b -> b `rem` 8 == 0) . values genb
...
-- 309
judge' :: Int -> Int -> Int
judge' inia inib = length . filter id . take 5000000 $ zipWith 
  (\a b -> mask .&. a == mask .&. b) (valuesa inia) (valuesb inib)

main :: IO ()
main = print $ judge' 289 629
